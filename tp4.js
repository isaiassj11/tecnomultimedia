...
function setup() {
  ...
  circuloJugador = crearCirculoJugador();
}
...
function jugar() {
  ...
  for (let i = 0; i < 5; i++) {
    circulos.push(crearCirculoAbajo());
  }
...
function crearCirculoAbajo() {
  let circulo = crearCirculo();
  circulo.y = random(height, height + 950);
  circulo.speed *= -1;
  return circulo;
}

function crearCirculoJugador() {
  return {
    x: mouseX,
    y: mouseY,
    diameter: 40,
